...
exports.getStoreBySlug = async (req, res, next) => {
  const store = await Store.findOne({ slug: req.params.slug }); //findOne(query)
...
exports.getStoresByTag = async (req, res) => {
  ...
  const tagQuery = tag || { $exists: true }; //if there is no tag, return all stores that have at least one tag
  const tagsPromise = Store.getTagsList(); //our own static method
  const storesPromise = Store.find({ tags: tagQuery }); //query for stores that have the tag
